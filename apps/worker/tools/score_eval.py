...
Reuses the exact production wiring (load_resumes -> make_claude_scorer("claude-sonnet-5")
...
label. PASS over the gate-eligible (non-`marked`) rows: 0 hard-invariant violations AND
...
def main() -> int:
    ...
    # max_tokens 8192 (prod is 4096): adaptive thinking + the verbose assessment notes
    # truncate the JSON at 4096 on some rows -> ScoreError. The cap doesn't change the
    # model's *score* (only whether the JSON completes), so this is measurement-neutral.
