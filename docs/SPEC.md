...
- **`pipeline.py` — orchestration.** Stateless stage functions over a db
  ...
  **A free seniority PRE-ORDERING runs first** (`score/seniority.py`, wired only on the
  ...
  (P 0.964, 44% demoted, **0 false demotions on `domain=match` and 0 on notified rows**)
  and the two keep-direction vetoes are docs/SCORING.md §5.7, along with what a green
  eval does NOT prove; the gate is `make eval-seniority`. An optional `max_id` (the
