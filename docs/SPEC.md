...
- **`score/` — screening + fit scoring.** A package — `screen.py`, `location.py`,
  ...
  **Quota telemetry (`score/usage.py`, free).** After a scoring pass that actually
  ...
  failure with its exception type, "not logged in", a 200 whose payload has no
  `rate_limit` object, a 200 whose windows carry a null `used_percent`, an unknown
  backend, and the blanket catch that covers the file write. The narrow `_get_json` catch
