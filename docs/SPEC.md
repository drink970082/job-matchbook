...
- **`pipeline.py` — orchestration.** Stateless stage functions over a db
  ...
  **Why the intake filters are re-applied here at all:** `prefilter_postings` runs at
  INGEST only, so a row that entered before its filter existed — or that simply aged past
  `max_age_days` while it waited in the queue — keeps its place and buys a PAID fit call
  on a posting this config refuses today. Measured 2026-07-31 against the live queue:
  **775 of the 5,941 rows that survive the deterministic gates**, 587 of them on AGE, 206
  on title, 18 on both — about 620 paid messages, over 30% of a weekly window. The
  disqualification reason is the same string the operator's 2026-07-29 manual sweep used,
  so both populations query alike. It runs only AFTER the location/intern gates, so a row
  those already killed keeps their reason rather than being relabelled.
...
| Phase 0 also re-applies the operator's CURRENT `title_filter`/`title_exclude`/`max_age_days` to already-queued rows, discarding them free with the `prefilter:` reason — but only after the location/intern gates, so a row they killed keeps its own reason; with no `stale_fn` the pass behaves exactly as before | `test_pipeline.py` (`test_a_row_the_current_filters_would_refuse_is_swept_free`, `test_the_stale_check_never_runs_on_a_row_a_gate_already_killed`, `test_no_stale_predicate_leaves_every_row_alone`) |
